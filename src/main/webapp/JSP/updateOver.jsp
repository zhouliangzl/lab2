    ...
    <div class="post-main">
        <div class="choose-item" style="width: 300px">
            <div class="post-over" style="color: white">
                <%--由servlet传送信息到这，判断修改成功或失败--%>
                <c:if test="${isUpdateSuccessed==true && not empty updateId}">修改成功</c:if>
                <c:if test="${isUpdateSuccessed==false}">修改失败</c:if>
            </div><br>

            <%--如果修改成功，并且是租房信息--%>
            <c:if test="${isUpdateSuccessed==true && postType==1}">
                <a href="/house/detile?hid=${updateId}">查看详情</a>
            </c:if>

            <%--如果修改成功，并且是二手商品信息--%>
            <c:if test="${isUpdateSuccessed==true && postType==2}">
                <a href="/usedGoods/detile?gid=${updateId}">查看详情</a>
            </c:if>
        </div>
    </div>
</div>
